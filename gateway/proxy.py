...
class ProxyRouter:
    ...
    def _record_failure(self, service_name: str) -> None:
        ...
        if state.failures >= CIRCUIT_OPEN_THRESHOLD and not state.is_open:
            ...
            logger.error("Circuit opened for %s after %d consecutive failures", service_name, state.failures)
    ...
    async def proxy(self, request: Request, route: ServiceRoute) -> Response:
        ...
        forwarded_headers.update({"X-Request-ID": request_id, "X-Forwarded-For": client_ip, "X-Forwarded-Host": request.headers.get("host", ""), "X-Gateway-Version": "1.0.0"})
        body = await request.body()
        upstream_request = self._client.build_request(method=request.method, url=upstream_url, headers=forwarded_headers, content=body)
        try:
            async for attempt in AsyncRetrying(
                ...
                with attempt:
                    upstream_response = await self._client.send(upstream_request, stream=True, timeout=route.timeout_seconds)
            self._record_success(route.name)
        except (httpx.ConnectError, httpx.TimeoutException) as exc:
            self._record_failure(route.name)
            logger.error("Upstream connection error for %s: %s", route.name, exc)
            return Response(content=f'{{"detail":"Could not connect to upstream service {route.name}"}}', status_code=502, media_type="application/json")
        except RetryError as exc:
            self._record_failure(route.name)
            logger.error("All retries exhausted for %s: %s", route.name, exc)
            return Response(content=f'{{"detail":"Upstream service {route.name} did not respond after retries"}}', status_code=502, media_type="application/json")
        response_headers = {k: v for k, v in upstream_response.headers.items() if k.lower() not in ("transfer-encoding", "connection", "content-encoding", "content-length")}
        ...
        return StreamingResponse(_stream_body(), status_code=upstream_response.status_code, headers=response_headers, media_type=upstream_response.headers.get("content-type", "application/json"))
